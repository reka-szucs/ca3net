...
if __name__ == "__main__":
    ...
    linear = True
    f_in = "spw_PC_spikes.npz"
    #f_in = "spike_trains_%.1f_linear.npz" % place_cell_ratio if linear else "spike_trains_%.1f.npz" % place_cell_ratio  #ezt átírni??
    ...
    spiking_neurons, spike_times = load_spike_trains(base_path / "files" / f_in)   # ezt kéne átírni???
    nPCs = int(np.max(spiking_neurons)) + 1
