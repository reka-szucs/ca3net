...
from helper import load_spike_trains, save_results_as_json, save_wmx
from plots import (
    plot_STDP_rule,
    plot_wmx,
    plot_wmx_avg,
...
if __name__ == "__main__":
    ...
    spiking_neurons, spike_times = load_spike_trains(
        os.path.join(base_path, "files", f_in)
    )
    ...
    file_name = os.path.join(base_path, "files", f_out)
    save_wmx(weightmx, file_name)
    content = {"weight_matrix": weightmx}
    save_results_as_json(content, file_name)
    # matrix = [row.tolist() for row in weightmx]
    # with open(os.path.join(base_path, "files", f_out + ".json"), "w") as f:
    ...
    plot_weights(
        save_selected_w(weightmx, selection), save_name="%s_sel_weights" % f_out[:-4]
    )
    plt.show()
